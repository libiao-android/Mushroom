...
import com.libiao.mushroom.mine.tab.BanFourTab
...
class BanFourFragment: BaseFragment(R.layout.fang_liang_fragment), MavericksView, ICommand {
    ...
    override fun order(type: Int, data: Any?) {
        when(type) {
            ...
            ICommand.LOCAL -> {
                ...
                if(!onLineChecked) {
                    fourBanViewModel.updateLocal()
                }

            }
            ICommand.NETWORK -> {
                mRefreshCount = 0
                fourBanViewModel.updateNetWork(this)
            }
        }
    }
    ...
    private val controller = BanFourController()
    private val fourBanViewModel: BanViewModel by fragmentViewModel()
    ...
    private fun initData() {
        loadingStatus = 1
        val data = BanShareDatabase.getInstance()?.getBanFourShareDao()?.getAllShares()

        fourBanViewModel.fetchInfo(toBanShareInfo(data))
    }

    private fun toBanShareInfo(data: MutableList<BanFourShareInfo>?): MutableList<BanShareInfo>? {
        data?.also {
    ...
    private fun initView() {
        ...
        fourBanViewModel.onEach(deliveryMode = UniqueOnly(UUID.randomUUID().toString())) {
            controller.setData(it)
        }
    }

    inner class BanFourController: TypedEpoxyController<BanState>() {
        ...
        override fun buildModels(data: BanState?) {
            ...
            if(size > 0) {
                loadingStatus = 0
                (activity as SelfSelectionActivity).notifyData(1, BanFourTab.TAG, size)
            }
            var time = ""
            data?.infoList?.forEach {
                if(it.time != time) {
                    ...
                    timeItemView {
                        ...
                        expand(it.expand)
                        click {v ->
                            fourBanViewModel.expand(it.time!!)
                        }
                    }
                }
                if(it.expand) {
                            ...
                            MoreDialog(context!!){view2 ->
                                when(view2.id) {
                                    R.id.btn_dialog_delete -> {
                                        ...
                                        fourBanViewModel.deleteItem(it)
                                    }
                                }
                            }.show()
                        }
    ...
    override fun shiShiRefresh() {
        mRefreshCount ++
        (activity as SelfSelectionActivity).notifyData(2, BanFourTab.TAG, mRefreshCount)
        LogUtil.i(TAG, "mRefreshCount: $mRefreshCount")
        if(mRefreshCount == size) {
            (activity as SelfSelectionActivity).notifyData(3, BanFourTab.TAG, 0)
        }
    }
}