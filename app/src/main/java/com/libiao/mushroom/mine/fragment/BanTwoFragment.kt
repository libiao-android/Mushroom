...
import com.libiao.mushroom.mine.tab.BanTwoTab
...
class BanTwoFragment: BaseFragment(R.layout.fang_liang_fragment), MavericksView, ICommand {
    ...
    override fun order(type: Int, data: Any?) {
        when(type) {
            ...
            ICommand.LOCAL -> {
                ...
                if(!onLineChecked) {
                    twoBanViewModel.updateLocal()
                }

            }
            ICommand.NETWORK -> {
                mRefreshCount = 0
                twoBanViewModel.updateNetWork(this)
            }
        }
    }
    ...
    private val controller = BanTwoController()
    private val twoBanViewModel: BanViewModel by fragmentViewModel()
    ...
    private fun initData() {
        loadingStatus = 1
        val data = BanShareDatabase.getInstance()?.getBanTwoShareDao()?.getAllShares()

        twoBanViewModel.fetchInfo(toBanShareInfo(data))
    }

    private fun toBanShareInfo(data: MutableList<BanTwoShareInfo>?): MutableList<BanShareInfo>? {
        data?.also {
    ...
    private fun initView() {
        ...
        twoBanViewModel.onEach(deliveryMode = UniqueOnly(UUID.randomUUID().toString())) {
            controller.setData(it)
        }
    }

    inner class BanTwoController: TypedEpoxyController<BanState>() {
        ...
        override fun buildModels(data: BanState?) {
            ...
            if(size > 0) {
                loadingStatus = 0
                (activity as SelfSelectionActivity).notifyData(1, BanTwoTab.TAG, size)
            }
            var time = ""
            data?.infoList?.forEach {
                if(it.time != time) {
                    ...
                    timeItemView {
                        ...
                        expand(it.expand)
                        click {v ->
                            twoBanViewModel.expand(it.time!!)
                        }
                    }
                }
                if(it.expand) {
    ...
    override fun shiShiRefresh() {
        mRefreshCount ++
        (activity as SelfSelectionActivity).notifyData(2, BanTwoTab.TAG, mRefreshCount)
        LogUtil.i(TAG, "mRefreshCount: $mRefreshCount")
        if(mRefreshCount == size) {
            (activity as SelfSelectionActivity).notifyData(3, BanTwoTab.TAG, 0)
        }
    }
}