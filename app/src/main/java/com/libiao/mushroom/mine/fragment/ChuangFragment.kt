...
import com.libiao.mushroom.mine.tab.ChuangTab
...
class ChuangFragment: BaseFragment(R.layout.xin_gao_fragment), ICommand {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        settingBean?.timeValue = 2
        initView()
    }

    private fun initData() {
        val data = ChuangDatabase.getInstance()?.getMineShareDao()?.getMineShares()
        data?.also {
            mData = it
            mTempData = it
        }
            ...
            when(mRangeStatus) {
                0 -> {
                    ...
                    when(currentCb) {
                        ...
                        4 -> mTempData = ArrayList(mTempData.sortedByDescending { it.nowPrice })
                ...
                1 -> {
                    ...
                    when(currentCb) {
                        ...
                        4 -> mTempData = ArrayList(mTempData.sortedBy { it.nowPrice })
            ...
            data.forEach {
                ...
                if(f.exists()) {
                    ...
                    if(it.candleEntryList == null || reload) {
                        ...
                        it.values_5.clear()
                        it.values_5.addAll(values_5)
                        it.values_10.clear()
                        it.values_10.addAll(values_10)
                        it.values_20.clear()
                        it.values_20.addAll(values_20)
...
//
    ...
    private fun refreshCount() {
        (activity as SelfSelectionActivity).notifyData(1, ChuangTab.TAG, mTempData.size)
    }
                    ...
                    when(it.id) {
                        R.id.btn_dialog_delete -> {
                            mineShareInfo?.code?.also {
                                ChuangDatabase.getInstance()?.getMineShareDao()?.delete(it)
...
//                }
        ...
        fun bindData(position: Int, info: MineShareInfo) {
            ...
            otherInfoTv?.text = "${huanSuanYi(info.maxPrice)}, ${info.maxCount}"
        ...
        private fun refreshTiCaiView(info: MineShareInfo) {
...
//        set1.formLineDashEffect = DashPathEffect(floatArrayOf(10f, 5f), 0f)
    ...
    override fun shiShiRefresh() {
        mRefreshCount ++
        (activity as SelfSelectionActivity).notifyData(2, ChuangTab.TAG, mRefreshCount)
        LogUtil.i(TAG, "mRefreshCount: $mRefreshCount")
        if(mRefreshCount == mTempData.size) {
            mAdapter?.setData(mTempData)
            (activity as SelfSelectionActivity).notifyData(3, ChuangTab.TAG, 0)
            resetSortUI()
        }
    }
}