...
import com.libiao.mushroom.mine.tab.FangLiang3Tab
...
class FangLiang3Fragment: BaseFragment(R.layout.xin_gao_fragment), ICommand {
    ...
    private fun initData() {
        val data = FangLiangShareDatabase3.getInstance()?.getFangLiangShareDao()?.getFangLiangShares()
        data?.also {
            mData = it
            mTempData = it
        }
            ...
            when(mRangeStatus) {
                0 -> {
                    ...
                    when(currentCb) {
                        ...
                        7 -> mTempData = ArrayList(mTempData.sortedByDescending { it.noXinGaoDay })
                    }
                ...
                1 -> {
                    ...
                    when(currentCb) {
                        ...
                        7 -> mTempData = ArrayList(mTempData.sortedBy { it.noXinGaoDay })
                    }
                    mAdapter?.setData(mTempData)
                }
                2 -> {
            ...
            data.forEach {
                ...
                if(f.exists()) {
                    ...
                    if (!hasRecord) {
                        ...
                        FangLiangShareDatabase3.getInstance()?.getFangLiangShareDao()?.update(it)
                    }
...
//            }
            ...
            if (info.delete == 1 && info.maxPrice < 3000000000) {
                if (cb_you_xuan.isChecked) {
                    continue
                }
            }

            if (info.maxPrice < 1000000000) {
                if (cb_you_xuan.isChecked) {
                    continue
                }
            }

            if (info.noXinGaoDay > 10 && info.maxPrice < 3000000000) {
                if (cb_you_xuan.isChecked) {
                    continue
                }
            }
...
//
    ...
    private fun refreshCount() {
        (activity as SelfSelectionActivity).notifyData(1, FangLiang3Tab.TAG, mTempData.size)
    }
                    ...
                    when(it.id) {
                        R.id.btn_dialog_delete -> {
                            mineShareInfo?.code?.also {
                                FangLiangShareDatabase3.getInstance()?.getFangLiangShareDao()?.delete(it)
...
//                }
        ...
        fun bindData(position: Int, info: FangLiangShareInfo) {
            ...
            if(info.currentCb == 7) {
                mRangeTv?.text = "${info.noXinGaoDay}"
                mRangeTv?.setTextColor(Color.parseColor("#f15b6c"))
            }
...
//        set1.formLineDashEffect = DashPathEffect(floatArrayOf(10f, 5f), 0f)
    ...
    override fun shiShiRefresh() {
        mRefreshCount ++
        (activity as SelfSelectionActivity).notifyData(2, FangLiang3Tab.TAG, mRefreshCount)
        LogUtil.i(TAG, "mRefreshCount: $mRefreshCount")
        if(mRefreshCount == mTempData.size) {
            mAdapter?.setData(mTempData)
            (activity as SelfSelectionActivity).notifyData(3, FangLiang3Tab.TAG, 0)
            resetSortUI()
        }
    }
}